...
app.get("/health", (req, res) => {
  res.json({ status: "ok", uptime: process.uptime(), cacheSize: m3u8Cache.size, version: "1.0.7" });
});
...
// ── M3U8 Proxy ────────────────────────────────────────────────────────────────
// Fusion/Infuse CDN'e Referer göndermediği için video açılmıyor.
// Bu endpoint CDN'e doğru headerlarla istek yapıp içeriği Fusion'a iletir.
app.get("/proxy/m3u8", async (req, res) => {
  ...
  try {
    const proxyRes = await httpGet(url, {
    ...
    // M3U8 içindeki segment URL'lerini de proxy'le
    let body = "";
    proxyRes.on("data", (chunk) => body += chunk);
    proxyRes.on("end", () => {
      // Relative URL'leri absolute yap, sonra proxy'den geçir
      const baseUrl = new URL(url);
      const lines = body.split("\n").map((line) => {
        line = line.trim();
        if (line.startsWith("#") || line === "") return line;
        // Segment veya alt playlist URL'si
        try {
          const segUrl = new URL(line, baseUrl.origin + baseUrl.pathname.substring(0, baseUrl.pathname.lastIndexOf("/") + 1)).toString();
          return `${CONFIG.ADDON_URL}/proxy/segment?url=${encodeURIComponent(segUrl)}`;
        } catch (e) { return line; }
      });
      res.send(lines.join("\n"));
    });
...
app.get("/stream/:type/:encodedId.json", async (req, res) => {
  ...
  try {
    ...
    // Proxy URL'si — addon kendi üzerinden CDN'e doğru headerlarla erişir
...
app.listen(CONFIG.PORT, "0.0.0.0", () => {
  console.log(`\n🚀 Fusion Dizipal Addon v1.0.7 -> http://0.0.0.0:${CONFIG.PORT}`);
  console.log(`   Addon URL: ${CONFIG.ADDON_URL}`);
});
